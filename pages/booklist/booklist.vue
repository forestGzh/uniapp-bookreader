<template>
    <view>
        <view class="book_item" v-for="(book, index) in books" :key="index">
            <navigator class="book_item_text" :url="'../chapters/chapters?bookHref=' + book.bookHref+'&bookName=' + book.bookName">
                {{book.bookName}}
            </navigator>
        </view>
        ...
        methods: {
            ...
            _requestPromise() {
                uni.request({
                    ...
                    var books = res[1].data;
                    // var books = res[1].data.slice(0,100);
                    this.books = books;
